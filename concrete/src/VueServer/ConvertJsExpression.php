        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            foreach ($expect as $ex) {
                ...
                if ($ex == 'value') {
                    ...
                    if (preg_match('/[a-zA-Z0-9_\!\[\{\(\"\']/', $char)) {
                        ...
                        if ($char == '{') {
                            ...
                            if ($this->inAttribute) {
                                $valueExpr .= 'ConvertJsExpression::handleArrayInAttribute("' . $this->inAttribute . '", ';
                            } else {
                                $valueExpr .= 'ConvertJsExpression::handleArrayToString(';
                            }
                            $valueExpr .= $subExpr->expr;
                            $valueExpr .= ')';
                            $i += $subExpr->length;
                            ...
                            if ($subChar == '(') {
                                ...
                                if (!empty($pathOfExpr)) {
                                    ...
                                    if (!empty($path)) {
                                        ...
                                        $valueExpr = $pre . 'ConvertJsExpression::getObjectValue(' . $pathOfExpr . ', "' . $path . '", $this)';
                                    } else {
                                ...
                                $subExpr = $parseBetween($i, '(', ')', true, false);
                                if ($funcName == 'indexOf') {
                                    $valueExpr = 'ConvertJsExpression::indexOf(' . $valueExpr . ', ' . $subExpr->expr . ')';
                                } else {
                                    $valueExpr .= $subExpr->expr;
                                }
                                $i += $subExpr->length;
                ...
                if ($ex == 'operator') {
                    ...
                    if ($operator === '+') {
                        $plus = true;
                        $newExpr = 'ConvertJsExpression::plus(' . $newExpr . ', ';
                    } elseif ($operator === '?') {
                        $newExpr = 'ConvertJsExpression::toBool(' . $newExpr . ') ?';
                    } else {
                        $newExpr .= $operator;
                    }
